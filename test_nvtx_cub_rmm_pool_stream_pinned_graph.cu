...
int main() {
    ...
    std::vector<raft::handle_t> handles(NB_IMAGES);
    std::vector<std::shared_ptr<rmm::device_uvector<int>>> d_matrices;
    std::vector<std::shared_ptr<rmm::device_uvector<int>>> d_medians;

    // Create CUDA Graph
    std::vector<cudaGraph_t> graphs(NB_IMAGES);
    std::vector<cudaGraphExec_t> graphExecs(NB_IMAGES);

    for (int i = 0; i < NB_IMAGES; ++i)
    {
        raft::handle_t& handle = handles[i];

        d_matrices.emplace_back(std::make_shared<rmm::device_uvector<int>>(MATRIX_SIZE, handles[i].get_stream()));
        d_medians.emplace_back(std::make_shared<rmm::device_uvector<int>>(NB_TILE_X * NB_TILE_Y, handles[i].get_stream()));

        rmm::device_uvector<int>& d_matrix = *d_matrices[i];
        rmm::device_uvector<int>& d_median = *d_medians[i];

        CUDA_CHECK_ERROR(cudaStreamBeginCapture(handle.get_stream(), cudaStreamCaptureModeGlobal));
        ...
        CUDA_CHECK_ERROR(cudaStreamEndCapture(handle.get_stream(), &graphs[i]));
        CUDA_CHECK_ERROR(cudaGraphInstantiate(&graphExecs[i], graphs[i], NULL, NULL, 0));
    }
...
#pragma omp parallel for
    for (int i = 0; i < NB_IMAGES; ++i)
    {
        ...
        CUDA_CHECK_ERROR(cudaGraphLaunch(graphExecs[i], handles[i].get_stream()));
        CUDA_CHECK_ERROR(cudaStreamSynchronize(handles[i].get_stream()));
    }
