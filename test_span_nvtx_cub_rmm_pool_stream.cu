...
#pragma omp parallel for
    for (int i = 0; i < NB_IMAGES; ++i)
    {
        ...
        // Copy memory to GPU
        CUDA_CHECK_ERROR(cudaMemcpyAsync(d_matrix.data(), h_matrices[i].data(), MATRIX_SIZE * sizeof(int), cudaMemcpyHostToDevice, stream.get()));
        ...
        CUDA_CHECK_ERROR(cudaMemcpyAsync(h_medians[i].data(), d_median.data(), (NB_TILE_X * NB_TILE_Y) * sizeof(int), cudaMemcpyDeviceToHost, stream.get()));
